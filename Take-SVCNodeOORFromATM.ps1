...
$servers = @(Read-Host "Enter Server Name or if have multiple servers. Please add followed by ',' comma.").Split(',')
$request = Read-Host "Action {Enable\Disable}?"

Foreach($server in $servers)
{
    $nodes | ? {$_.userNote -eq $server.Trim()} | ForEach-Object # have scope for parallel execution if require
    {
    If($_.StatusEnabledState -eq 'enabled' -and $request -eq 'Disable') 
    {
        Try
        {
            $Status = Invoke-WFLTMVIP.ps1 -Activity UPdateNode -NodeName $_.name -VipName $VipName -NetworkType $NetworkType -DataCenter $DataCenter -Area $Area -Body @"
{"Status": "Disabled"}
"@    
            ...
            {
                Write-Host -ForegroundColor red "Failed : $($Status)"
            }
        }
        catch
        {
            Write-Host -ForegroundColor red "Failed : $($Status) Error - $($Error[0].Exception)."
        }
    }
    elseif ($_.StatusEnabledState -eq 'Disabled' -and $request -eq 'Enable') 
    {
        Try
        {
            $Status = Invoke-WFLTMVIP -Activity UPdateNode -NodeName $_.name -VipName $VipName -NetworkType $NetworkType -DataCenter $DataCenter -Area $Area -Body @"
{"Status": "enabled"}
"@   
            Write-Host -ForegroundColor Cyan "Information : Putting Node {$($_.usernote)} back on rotation into ATM Portal."
            if($Status.status -eq 'Enabled')
            {
                Start-Sleep -Seconds 2
                Write-Host -ForegroundColor Cyan "Information - Node : {$($_.usernote)} Successfully Enabled"
            }
            else
            {
                Write-Host -ForegroundColor Red "Warning - Node : {$($_.usernote)} Failed to Enable."
            }
        }
        catch
        {
            Write-Host -ForegroundColor Red "Warning - Node : {$($_.usernote)} Failed to Enable. Error - {$($Error[0].Exception)}"
        }
    }
    else {
